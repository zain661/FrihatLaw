...
import { navEn, contactEn, contactLinkEn } from "../data/shared.en";
...
function FooterLink({ to, children }) {
  return (
    <Link
      to={to}
      className="relative inline-block w-fit text-cream/80 transition-colors duration-300 hover:text-cream after:absolute after:-bottom-1 after:start-0 after:h-px after:w-0 after:bg-gold-light after:transition-all after:duration-300 hover:after:w-full"
    >
      {children}
    </Link>
  );
...
// Per-entity phone/email shown under each entity name in the entities column.
...
const COPY = {
  ar: {
    tagline: "منظومة مؤسسية فلسطينية تجمع الخبرة القانونية، وحماية الملكية الفكرية، وحلول الموارد البشرية تحت مظلة واحدة.",
    entities: "كياناتنا",
    quickLinks: "روابط سريعة",
    contactUs: "تواصل معنا",
    rights: "© 2026 فريحات جروب. جميع الحقوق محفوظة.",
  },
  en: {
    tagline: "A Palestinian institutional ecosystem bringing together legal expertise, intellectual property protection, and HR solutions under one umbrella.",
    entities: "Our Entities",
    quickLinks: "Quick Links",
    contactUs: "Contact Us",
    rights: "© 2026 Frihat Group. All rights reserved.",
  },
};
...
export default function Footer() {
  ...
  const activeContactLink = lang === "en" ? contactLinkEn : contactLink;
  ...
  const quickLinks = [
    ...["/#about", "/frihat-legal", "/blog"].map((to) => activeNav.find((item) => item.to === to)),
    activeContactLink,
  ];
      ...
      <div className="relative mx-auto max-w-[1200px]">
        <div className="grid gap-12 lg:grid-cols-12 pb-14 border-b border-cream/10">
          <div className="lg:col-span-4">
            <Link to="/" className="flex items-center gap-3 mb-5 w-fit">
              ...
              <span className="flex flex-col leading-tight font-head">
                <strong className="text-base font-extrabold text-cream">{lang === "en" ? "Frihat" : brand.nameAr}</strong>
                <em className="not-italic text-[0.62rem] tracking-[0.3em] text-gold-light font-bold">
                  {brand.nameEn.split(" ")[1]}
                </em>
              </span>
            </Link>
            <p className="text-sm leading-relaxed text-cream/75 max-w-xs">{t.tagline}</p>
          </div>

          <div className="lg:col-span-3">
            <p className="font-head text-sm tracking-widest leading-relaxed text-gold-light font-semibold uppercase mb-5">{t.entities}</p>
                  ...
                  <li key={c.id}>
                    ...
                    {entityContact && (
                      <div className="mt-1 flex flex-col gap-0.5 text-xs text-cream/55">
                        <a href={`tel:${entityContact.phone.replace(/[\s-]/g, "")}`} dir="ltr" className={`w-fit ${numAlign} hover:text-gold-light transition-colors`}>
                          {entityContact.phone}
                        </a>
                        <a href={`mailto:${entityContact.email}`} dir="ltr" className={`w-fit ${numAlign} hover:text-gold-light transition-colors`}>
                          {entityContact.email}
                        </a>
                      </div>
                    )}
          ...
          <div className="lg:col-span-2">
            <p className="font-head text-sm tracking-widest leading-relaxed text-gold-light font-semibold uppercase mb-5">{t.quickLinks}</p>
          ...
          <div className="lg:col-span-3">
            <p className="font-head text-sm tracking-widest leading-relaxed text-gold-light font-semibold uppercase mb-5">{t.contactUs}</p>
            <ul className="space-y-4 text-sm">
              <li className="flex items-start gap-2.5">
                ...
                <span className="text-cream/80 leading-relaxed">{activeContact.address}</span>
              </li>
              <li className="flex items-start gap-2.5">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4 mt-0.5 shrink-0 text-gold-light/70">
                ...
                <div className="flex flex-col gap-1">
                  {activeContact.phones.map((p) => (
                    <a key={p} href={`tel:${p.replace(/\s/g, "")}`} dir="ltr" className={`text-cream/80 hover:text-cream transition-colors ${numAlign}`}>
                      {p}
                    </a>
                  ))}
                </div>
              ...
              <li className="flex items-center gap-2.5">
                ...
                <a href={`mailto:${activeContact.email}`} dir="ltr" className="text-cream/80 hover:text-cream transition-colors">
                  {activeContact.email}
                </a>
              </li>
            </ul>
