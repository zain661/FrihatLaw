...
const HUB_NODES = [
  { icon: "fingerprint", label: "بصمة و GPS", pos: "top-0 right-0", line: [80, 12] },
  { icon: "payments", label: "مسير الرواتب", pos: "top-0 left-0", line: [20, 12] },
  { icon: "event_available", label: "الإجازات", pos: "bottom-0 right-0", line: [80, 88] },
  { icon: "insights", label: "التقارير", pos: "bottom-0 left-0", line: [20, 88] },
];
  ...
  return (
    <section className="relative z-20 px-5 -mt-8 pb-10 md:px-16 md:pb-12">
      <Reveal className="relative z-20 mx-auto max-w-6xl rounded-3xl border border-sky-100 bg-white/90 p-6 shadow-xl backdrop-blur md:p-8">
        <div dir="rtl" className="grid grid-cols-2 gap-6 md:grid-cols-4">
          {data.map((s, i) => (
...
function FeatureRow({ tag, title, desc, mockupKey, reverse }) {
  ...
  return (
    <Reveal className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
      ...
      <div dir="rtl" className={`text-center lg:text-right ${reverse ? "lg:order-1" : ""}`}>
...
function CompanySizeTabs({ data }) {
  const [active, setActive] = useState(0);
  const current = data[active];

  return (
    <div>
      <div dir="rtl" className="mb-10 flex flex-wrap justify-center gap-3">
        {data.map((seg, i) => (
          <button
            key={seg.key}
            type="button"
      ...
      <motion.div
        ...
        dir="rtl"
        className="mx-auto max-w-3xl rounded-3xl border border-[#0B5FA5]/15 bg-white p-8 text-right shadow-[0_25px_60px_-25px_rgba(11,34,85,0.3)] md:p-10"
      >
...
function AutomationHub() {
  return (
    <div className="relative mx-auto w-full max-w-[400px] aspect-square">
      <div className="absolute -top-8 -right-8 h-52 w-52 rounded-full bg-[#06BAEB]/30 blur-[70px]" />
      <div className="absolute -bottom-8 -left-8 h-52 w-52 rounded-full bg-[#241F4E]/70 blur-[70px]" />
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <div dir="rtl" className="relative mx-auto hidden max-w-4xl md:block">
        <div className="absolute right-1/2 top-0 h-full w-[3px] translate-x-1/2 rounded-full bg-[#1E293B]/10" />
        <motion.div
          style={{ scaleY: lineScale }}
          className="absolute right-1/2 top-0 h-full w-[3px] origin-top translate-x-1/2 rounded-full bg-gradient-to-b from-gold-light to-gold shadow-[0_0_20px_rgba(212,175,55,0.6)]"
        />
          ...
          {data.steps.map((step, i) => {
            ...
            return (
              <Reveal key={step.title} className={`relative flex ${isStart ? "justify-start" : "justify-end"}`}>
                ...
                <div className="absolute right-1/2 top-6 flex h-12 w-12 translate-x-1/2 items-center justify-center rounded-full border-2 border-gold-light bg-gradient-to-br from-[#2563EB] to-[#1E40AF] font-head text-sm font-bold text-gold-light shadow-[0_0_25px_rgba(231,184,77,0.55)]">
      ...
      <div dir="rtl" className="relative md:hidden">
        <div className="absolute right-5 top-0 h-full w-[3px] rounded-full bg-[#1E293B]/10" />
        <motion.div
          style={{ scaleY: lineScale }}
          className="absolute right-5 top-0 h-full w-[3px] origin-top rounded-full bg-gradient-to-b from-gold-light to-gold shadow-[0_0_16px_rgba(212,175,55,0.5)]"
        />
        <div className="flex flex-col gap-8">
          {data.steps.map((step, i) => (
            <Reveal key={step.title} delay={i * 0.05} className="relative pr-16">
              <div className="absolute right-0 top-0 flex h-10 w-10 items-center justify-center rounded-full border-2 border-gold-light bg-gradient-to-br from-[#2563EB] to-[#1E40AF] font-head text-xs font-bold text-gold-light shadow-[0_0_18px_rgba(231,184,77,0.5)]">
                {String(i + 1).padStart(2, "0")}
              </div>
...
export default function NHRPage() {
  return (
    <div className="bg-[#f9f9fc]">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div className="flex items-center gap-3 mb-6">
              <span className="h-9 w-9 rounded-xl overflow-hidden shadow-lg shrink-0">
                <img src="/brand/logo-nhr-icon.png" alt="كيان NHR" className="h-full w-full object-cover" />
              </span>
              <span className="font-body text-sm md:text-base font-semibold tracking-widest leading-relaxed uppercase text-[#8FF3FF]">{nhr.tag}</span>
            </div>
            <h1 className="font-head text-4xl md:text-6xl font-bold leading-tight mb-6">
              كيان NHR{" "}
              <span className="bg-gradient-to-l from-[#22D3EE] to-[#B6F9FF] bg-clip-text text-transparent">{nhr.tagline}</span>
            </h1>
            <p className="font-body text-lg text-white/80 leading-relaxed mb-9 max-w-xl">{nhr.heroTagline}</p>
      ...
      <section id="services" className="scroll-mt-24 py-10 md:py-12 bg-[#f9f9fc]">
        <div className="max-w-[1280px] mx-auto px-5 md:px-16">
          <Reveal className="text-center mb-20">
            <h2 className="font-head text-3xl md:text-4xl font-bold text-[#003d7c] mb-4">{nhr.solutions.title}</h2>
            <p className="font-body text-[17px] text-[#424751] max-w-xl mx-auto">{nhr.solutions.subtitle}</p>
            <div className="h-1 w-24 bg-[#06BAEB] mx-auto mt-5" />
          </Reveal>

          <div className="flex flex-col gap-20 md:gap-28">
            {nhr.featureShowcase.map((f, i) => (
              <FeatureRow key={f.key} tag={f.tag} title={f.title} desc={f.desc} mockupKey={f.mockup} reverse={i % 2 === 1} />
            ))}
          </div>

          <Reveal className="mt-24 grid grid-cols-2 gap-4 md:grid-cols-4">
            {nhr.quickFacts.map((f) => (
              <div
                key={f.label}
          ...
          <Reveal delay={0.1} className="mt-14 text-center">
            ...
            >
              استكشف كافة الخدمات والتفاصيل الشاملة
      ...
      {/* Security & Compliance */}
      <ComplianceSection data={nhr.trust} />

      {/* Company size — scales with you */}
      <section className="py-10 md:py-12 bg-white px-5 md:px-16">
        <div className="max-w-[1280px] mx-auto">
          <Reveal className="text-center max-w-2xl mx-auto mb-14">
            <h2 className="font-head text-3xl md:text-4xl font-bold text-[#003d7c] mb-4">يتوسّع معك مهما كان حجم فريقك</h2>
            <p className="font-body text-[17px] text-[#475569] leading-relaxed">اختر حجم مؤسستك لترى كيف تتكيّف كيان NHR مع احتياجاتك</p>
            <div className="h-1 w-24 bg-[#06BAEB] mx-auto mt-5" />
          </Reveal>

          <CompanySizeTabs data={nhr.companySizes} />
        </div>
      </section>
      ...
      <section className="pt-10 pb-10 md:pt-12 md:pb-12 bg-[#f9f9fc] px-5 md:px-16">
        <div className="max-w-[1100px] mx-auto">
          <Reveal className="text-center max-w-2xl mx-auto mb-6">
            <h2 className="font-head text-3xl md:text-4xl font-bold text-[#1E293B] mb-4">{nhr.process.title}</h2>
            <p className="font-body text-[17px] text-[#475569] leading-relaxed">{nhr.process.subtitle}</p>
            <div className="h-1 w-24 bg-gold-light mx-auto mt-5" />
          </Reveal>

          <ProcessTimeline data={nhr.process} />
        </div>
      </section>
      ...
      <section className="py-10 md:py-12 bg-white px-5 md:px-16">
        <div className="max-w-[1280px] mx-auto">
          <Reveal className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="font-head text-3xl md:text-4xl font-bold text-[#003d7c]">لماذا يثق بنا عملاؤنا</h2>
            <div className="h-1 w-20 bg-[#06BAEB] mx-auto mt-4" />
          </Reveal>

          <div className="grid sm:grid-cols-2 gap-6 mb-14">
            {nhr.whyUs.items.map((it, i) => (
              <Reveal
                key={it}
                delay={i * 0.08}
                className="flex items-start gap-4 p-6 rounded-xl border-t-4 border-[#00658d] bg-[#f9f9fc]"
              >
          ...
          <Reveal delay={0.2} className="text-center">
            <p className="font-body text-sm md:text-base font-semibold text-[#003d7c] uppercase tracking-widest leading-relaxed mb-5">{nhr.clients.title}</p>
            <div className="flex flex-wrap justify-center gap-3">
              {nhr.clients.items.map((c) => (
                <button
                  key={c}
                  type="button"
      ...
      <section className="relative overflow-hidden px-5 md:px-10 py-10 md:py-12 bg-[#f9f9fc]">
        <div className="max-w-[1200px] mx-auto">
          <Reveal className="relative overflow-hidden rounded-[36px] bg-[#1E293B] px-8 py-16 md:px-16 md:py-20 text-white text-center">
            ...
            <div className="pointer-events-none absolute -bottom-1/3 -right-1/4 h-[500px] w-[500px] rounded-full bg-sky-500/25 blur-[130px]" />
            <div className="pointer-events-none absolute -top-1/3 -left-1/4 h-[400px] w-[400px] rounded-full bg-sky-400/15 blur-[120px]" />
            <div className="relative max-w-xl mx-auto">
              <div className="inline-flex h-12 w-12 rounded-2xl overflow-hidden shadow-lg mb-6 ring-2 ring-sky-400/40">
                <img src="/brand/logo-nhr-icon.png" alt="" className="h-full w-full object-cover" />
              </div>
              <h2 className="font-head text-2xl md:text-4xl font-extrabold leading-tight mb-5">{nhr.cta.title}</h2>
              {nhr.cta.body && <p className="text-white/70 text-lg leading-relaxed mb-10">{nhr.cta.body}</p>}
